...
def obtener_playlist_info(link):
    try:
        result = subprocess.run([os.path.join(sys._MEIPASS, "yt-dlp.exe"), "--flat-playlist", "--dump-json", link],
            capture_output=True, text=True)
        return [json.loads(line) for line in result.stdout.strip().split("\n") if line]
    except:
        return []
...
# 🏷️ Cambiar nombre de la ventana y aplicación
root.title("visual yt-dlp")
root.wm_title("visual yt-dlp")

# 🖼️ Cargar ícono personalizado (asegurate de que 'icon.ico' esté en el mismo directorio)
try:
